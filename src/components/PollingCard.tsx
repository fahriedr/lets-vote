...
function PollingCard<T>() {
    const [options, setOptions] = useState([""])
    const [mulitpleOptionToggle, setMulitpleOptionToggle] = useState(false)
        ...
        <div className="flex w-full lg:w-[50%] rounded overflow-hidden shadow-lg bg-[#393E46] px-8 py-8">
            <div className='flex w-full flex-col gap-6'>
                <div className="flex flex-col w-full">
                    <label className="tracking-wider text-sm mb-1 block font-bold text-white">Title</label>
                    <input
                        type="text"
                ...
                <div className="flex flex-col w-full gap-2">
                    <label className="tracking-wider text-sm mb-1 block font-bold text-white">Options</label>
                ...
                <div className='flex flex-col pt-3 pb-8 w-full'>
                    ...
                    <div className='flex flex-col gap-4 w-1/2'>
                        <div className="flex items-center justify-between">
                            <span className="text-gray-200">Allow select multiple options</span>
                            <div
                                onClick={toggleMultipleOptionSwitch}
                                className={`w-12 h-6 flex items-center rounded-full p-1 cursor-pointer transition-colors ${mulitpleOptionToggle ? 'bg-blue-500' : 'bg-gray-500'
                                    }`}
                            >
                                <div
                                    className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${mulitpleOptionToggle ? 'translate-x-6' : 'translate-x-0'
                                        }`}
                                ></div>
                            </div>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-gray-200 text-base font-medium">Allow comment</span>
                            <div
                        ...
                        <div className="flex items-center justify-between">
                            <span className="text-gray-200 text-base font-medium">Close poll with schedule date</span>
                            <div
                    ...
                    <div className="flex items-center space-x-2 pt-6">
                        <button
                            className="flex items-center px-16 py-2 bg-blue-500 text-gray-200 rounded-md hover:bg-gray-600 transition font-semibold text-base"
                        >
                            Create Poll
                        </button>
                    </div>
